...
int find_line_of_user_searched(void)
{
	...
	//init
	strcpy(user_searched.name, "0");
	strcpy(user_searched.sex, "0");
	strcpy(user_searched.age, "0");
	strcpy(user_searched.id, "0");
	strcpy(user_searched.number, "0");
	strcpy(user_searched.job, "0");
	strcpy(user_searched.address, "0");
	fscanf(fp_c,"%s",user_searched.name);
	fscanf(fp_c,"%s",user_searched.sex);
	fscanf(fp_c,"%s",user_searched.age);
	fscanf(fp_c,"%s",user_searched.id);
	fscanf(fp_c,"%s",user_searched.number);
	fscanf(fp_c,"%s",user_searched.job);
	fscanf(fp_c,"%s",user_searched.address);
	fclose(fp_c);
	while(!feof(fp_u))
	{
		//init
		strcpy(now_user.name, "0");
		strcpy(now_user.sex, "0");
		strcpy(now_user.age, "0");
		strcpy(now_user.id, "0");
		strcpy(now_user.number, "0");
		strcpy(now_user.job, "0");
		strcpy(now_user.address, "0");
		fscanf(fp_u,"%s",now_user.name);
		fscanf(fp_u,"%s",now_user.sex);
		fscanf(fp_u,"%s",now_user.age);
		fscanf(fp_u,"%s",now_user.id);
		fscanf(fp_u,"%s",now_user.number);
		fscanf(fp_u,"%s",now_user.job);
		fscanf(fp_u,"%s",now_user.address);
		line++;
		if(strcmp(now_user.name,user_searched.name)==0&&
			strcmp(now_user.sex,user_searched.sex)==0&&
			strcmp(now_user.age,user_searched.age)==0&&
			strcmp(now_user.id,user_searched.id)==0&&
			strcmp(now_user.number,user_searched.number)==0&&
			strcmp(now_user.job,user_searched.job)==0&&
			strcmp(now_user.address,user_searched.address)==0)
		{
			fclose(fp_u);
			return line;
		}
...
void task_modify_users(void)
{
	...
	while(!sure_flag)
	{
		if(strcmp(user_searched.sex, "0")==0)
		{
			break;
		}
		system("cls");
	...
	if(strcmp(user_searched.sex, "0")!=0)
	{
		...
		fprintf(fp, "%-20s\t%-4s\t%-3s\t%-18s\t%-11s\t%-40s\t%-40s\t\n", 
			user_searched.name,user_searched.sex,user_searched.age,user_searched.id,user_searched.number,user_searched.job,user_searched.address);
		fclose(fp);
		printf("finished!\n");
		fflush(stdin);
		scanf("%c", &quit);
